...
class EntityProperties(unittest.TestCase):
    ...
    def test_pavlohrad(self):
        prop_value = self.kdb.get_entity_properties('http://dbpedia.org/resource/Pavlohrad')
        given = prop_value['http://dbpedia.org/ontology/maximumElevation']
        expected = ['71.0']
        self.assertEqual(given, expected)

    def test_new_york(self):
        prop_value = self.kdb.get_entity_properties('http://dbpedia.org/resource/New_York')
